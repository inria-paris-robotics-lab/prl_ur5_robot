...
#
# A script is necessary (rather than another launchfile) since it reads arguments in the parameter server.
########
...
gripper_type=`rosparam get /setup/$1/gripper`
gripper_controller=`rosparam get /setup/$1/gripper_controller`

if [ "$gripper_controller" = "onrobot_rg" ]
then
    roslaunch onrobot_control onrobot_gripper.launch gripper_type:=$gripper_type $arg_to_forward __ns:="$1_gripper"
